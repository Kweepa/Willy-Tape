...
def format_udg(data: bytes, extra: Mapping[str, Any]) -> list[str]:
    ...
    for bit, name in optional:
        if flags & bit:
            ...
            pos += 8
    if pos < len(data):
        lines.extend(byte_lines(data[pos:]))
    return lines
...
def write_catalogue_asm(
    *,
    ...
    nightmare_set_idx = report.get("nightmare_player_sprite_set_idx", 0)
    sprite_lines: list[str] = [
        ...
        f"nightmare_player_sprite_set_idx = {nightmare_set_idx}  ; bat (8 frames)",
        "",
        "sprite_set_metadata",
    ...
    sprite_lines.extend(
        [
            ...
            "; Flat pool layout — runtime indexes by frame number (see set metadata above).",
        ]
    )
    set_starts: dict[int, str] = {}
    for start, count, name in pool.sets:
        if start not in set_starts:
            set_starts[start] = f"{name} ({count} frames @ {start}..{start + count - 1})"
    for idx in range(pool.frame_count):
        if idx in set_starts:
            sprite_lines.append(f"; --- {set_starts[idx]} ---")
        off = idx * 32
