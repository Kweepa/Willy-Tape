...
| `@arrow` | y v | Optional flying arrow (one per room). **Mutually exclusive with `@rope`.** `y` = entity pixel Y (tile row = `y >> 3`); `v` = `1` (LTR) or `-1` (RTL). Start column and launch SFX column are runtime constants (LTR: x=80, sound=115; RTL: x=40, sound=36). Catalogue stores one packed byte: ConvertXY row in bits 7–1, direction in bit 0. Unified **`arrow_update`** in PRG; UDGs @ chr **64–65** (`arrow_udgs.asm`). Max **4 `@guardians`**. Meta flag at tail +99. Legacy `x`/`sound` fields in `@arrow` are accepted but ignored. |
| `@arrowudg` | bytes | Optional 8-byte custom arrow UDG (tape port uses resident chr 64–65 instead). |
